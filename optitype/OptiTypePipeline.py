...
def main():
    ...
    COMMAND = "-i 97 -m 99999 --distance-range 0 -pa -tc %s -o %s.bam %s %s"
    ...
    ALLELE_HDF = rel_to_abs('data/alleles.h5')
    MAPPING_REF = {'gen': rel_to_abs('data/hla_reference_dna.fasta'),
                   'nuc': rel_to_abs('data/hla_reference_rna.fasta')}
    ...
    bam_paths = args.input if bam_input else [os.path.join(out_dir, ("%s_%i.bam" % (date, i))) for i in range(len(args.input))]
